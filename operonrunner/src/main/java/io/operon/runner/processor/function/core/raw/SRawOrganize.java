...
// Output: Array
//
// NOTE: this is same code as in StringOrganize
//
public class SRawOrganize extends BaseArity1 implements Node, Arity1 {
    
    public SRawOrganize(Statement statement, List<Node> params) throws OperonGenericException {
        super(statement);
                ...
                for (int i = 0; i < options.getPairs().size(); i ++) {
                    ...
                    switch (pair.getKey().toLowerCase()) {
                        ...
                        case "\"readfirst\"":
                            ...
                            break;
                        case "\"charset\"":
                            String charSet = ((StringType) pair.getValue().evaluate()).getJavaStringValue();
                            charset = Charset.forName(charSet.toUpperCase());
                            break;
                    }
                }
            }
            
            String strValue = new String(raw.getBytes(), charset);
