...
public class StringLength extends BaseArity0 implements Node, Arity0 {
    
    public StringLength(Statement statement) {
        super(statement);
    ...
    public NumberType evaluate() throws OperonGenericException {        
        try {
            ...
            int resultLength = str.getJavaStringValue().length();

            NumberType result = new NumberType(this.getStatement());
            result.setDoubleValue((double) resultLength);
