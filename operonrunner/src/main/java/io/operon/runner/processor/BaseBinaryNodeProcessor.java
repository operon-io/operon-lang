...
public abstract class BaseBinaryNodeProcessor implements BinaryNodeProcessor, java.io.Serializable {
    protected static Logger log = LogManager.getLogger(BaseBinaryNodeProcessor.class);
    ...
    public void preprocess(Statement statement, Node lhs, Node rhs) throws OperonGenericException {
        ...
        if (lhsResult instanceof OperonValue) {
            ...
            //   the items in the structure. When done e.g. in For -pattern, this would
            //   slow down Operon. Example: 'Select [] For ($i: 1000): @ + 1;'
    ...
    public OperonValue process(Statement statement, Node lhs, Node rhs) throws OperonGenericException {
        log.debug("BaseBinaryNodeProcessor :: process :: Returning null");
        return null;
    }
    ...
    public boolean customBindingCheck(Node lhs, Node rhs, String binaryOperator) {
        ...
        if (rhsResult != null) {
            log.debug("customBindingCheck :: rhs -> doBindings() :: " + rhs.getDoBindings());
            log.debug("customBindingCheck :: rhsResult -> doBindings() :: " + rhsResult.getDoBindings());
            log.debug("customBindingCheck :: rhsResult.getBindings().get(binaryOperator) != null :: " + rhsResult.getBindings().get(binaryOperator) != null);
            log.debug("customBindingCheck :: rhsResult.getBindings().size() :: " + rhsResult.getBindings().size());
        }
    ...
    public OperonValue doCustomBinding(Statement statement, Node lhs, Node rhs, String binaryOperator) throws OperonGenericException {
        ...
        if (op != null) {
            ...
            log.debug("doCustomBinding :: invoke binary function");
            OperonValue result = (OperonValue) fRef.invoke();
            log.debug("doCustomBinding :: done invoking binary function");
