...
public abstract class BaseUnaryNodeProcessor implements UnaryNodeProcessor, java.io.Serializable {
    private static Logger log = LogManager.getLogger(BaseUnaryNodeProcessor.class);
    
    protected OperonValue nodeResult;
    
    public void preprocess(Statement statement, Node node) throws OperonGenericException {
        log.debug("BaseUnaryNodeProcessor :: preprocess");
        OperonValue initialValue = statement.getCurrentValue();
        nodeResult = node.evaluate();
        log.debug("  BaseUnaryNodeProcessor :: nodeResult bindings size :: " + nodeResult.getBindings().size());
        // Fork the initial value for rhs:
        statement.setCurrentValue(initialValue);
        
        if (nodeResult instanceof OperonValue) {
            log.debug("  BaseUnaryNodeProcessor :: noderesult is OperonValue --> unboxing");
            log.debug("  BaseUnaryNodeProcessor :: noderesult bindings size before unboxing :: " + nodeResult.getBindings().size());
            ...
            log.debug("  BaseUnaryNodeProcessor :: nodeResult bindings size after unboxing :: " + nodeResult.getBindings().size());
        }
    }
    
    public OperonValue process(Statement statement, Node lhs, Node rhs) throws OperonGenericException {
        log.debug("BaseUnaryNodeProcessor :: process :: Returning null");
        return null;
    }

    public boolean customBindingCheck(Node node, String unaryOperator) {
        log.debug("customBindingCheck :: node -> doBindings() :: " + node.getDoBindings());
        log.debug("customBindingCheck :: node.getBindings().get(unaryOperator) != null :: " + node.getBindings().get(unaryOperator) != null);
        log.debug("customBindingCheck :: node.getBindings().size() :: " + node.getBindings().size());
        if ( (node.getBindings().size() > 0 && node.getDoBindings() == true && node.getBindings().get(unaryOperator) != null) ) {
            log.debug("customBindingCheck :: yes, do bindings");
            return true;
        }
        
        else {
            log.debug("customBindingCheck :: don't do bindings");
            return false;
        }
    }
    ...
    public OperonValue doCustomBinding(Statement statement, Node node, String unaryOperator) throws OperonGenericException {
        ...
        if (op != null) {
            ...
            log.debug("doCustomBinding :: INVOKE UNARY FUNCTION !!!!!!!");
            OperonValue result = (OperonValue) fRef.invoke();
            log.debug("doCustomBinding :: DONE INVOKING UNARY FUNCTION !!!!!!!");
