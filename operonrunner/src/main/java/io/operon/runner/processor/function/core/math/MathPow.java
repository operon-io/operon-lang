...
public class MathPow extends BaseArity1 implements Node, Arity1 {

    public MathPow(Statement statement, List<Node> params) throws OperonGenericException {
        super(statement);
        this.setParam1AsOptional(true);
        this.setParams(params, "pow", "exp");
        this.setNs(Namespaces.MATH);
    }
    ...
    public NumberType evaluate() throws OperonGenericException {        
        try {
            ...
            double exponentDouble = ((NumberType) this.getParam1().evaluate()).getDoubleValue();
            
            double powValue = Math.pow( ((NumberType) currentValue).getDoubleValue(), exponentDouble );
            ...
            result.setDoubleValue(powValue);
            return result;
        } catch (Exception e) {
