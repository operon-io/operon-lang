...
public class TestsCompiler extends OperonTestsBaseListener {
    ...
    public TestsCompiler() throws OperonGenericException, IOException {
        ...
        //:OFF:log.debug("TestsCompiler :: created");
    }
    ...
    public void exitAssert_component_stmt(OperonTestsParser.Assert_component_stmtContext ctx) {
        //:OFF:log.debug("exitAssert_component_stmt :: stack size :: " + this.stack.size());
    ...
    public void exitMock_stmt(OperonTestsParser.Mock_stmtContext ctx) {
        //:OFF:log.debug("exitMock_component_stmt :: stack size :: " + this.stack.size());
    ...
    public void exitValue_ref(OperonTestsParser.Value_refContext ctx) {
        //:OFF:log.debug("TestsCompiler :: EXIT Value_ref :: Stack size :: " + this.stack.size());
    ...
    public void exitJson_value(OperonTestsParser.Json_valueContext ctx) {
        ...
        if (subNodes.size() > 0 
            ...
            && subNodes.get(0) instanceof RuleNode 
            && subNodes.get(0) instanceof OperonTestsParser.Path_valueContext) {
            ...
            && subNodes.get(0) instanceof RuleNode 
            && subNodes.get(0) instanceof OperonTestsParser.Compiler_obj_config_lookupContext) {
            ...
            // """
            //
            else if ( (token = ctx.getToken(OperonTestsParser.MULTILINE_STRING, 0)) != null) {
                StringType sNode = new StringType(this.currentStatement);
                String symbolText = token.getSymbol().getText();
                //:OFF:log.debug("TerminalNode: String. Text :: " + symbolText);
            ...
            // """|
            // 
            else if ( (token = ctx.getToken(OperonTestsParser.MULTILINE_STRIPPED_STRING, 0)) != null) {
                StringType sNode = new StringType(this.currentStatement);
                String symbolText = token.getSymbol().getText();
                //:OFF:log.debug("TerminalNode: String. Text :: " + symbolText);
            ...
            // """>
            //
            else if ( (token = ctx.getToken(OperonTestsParser.MULTILINE_PADDED_LINES_STRING, 0)) != null) {
    ...
    @Override
    public void exitObj_update_expr(OperonTestsParser.Obj_update_exprContext ctx) {
    ...
    @Override
    public void exitIo_call(OperonTestsParser.Io_callContext ctx) {
        //:OFF:log.debug("EXIT Io_call :: Stack size :: " + this.stack.size());
        List<ParseTree> nodes = getContextChildNodes(ctx);

        IOCall ioCall = new IOCall(this.getCurrentStatement());
    ...
    @Override
    public void exitOperon_type_function_shortcut(OperonTestsParser.Operon_type_function_shortcutContext ctx) {
